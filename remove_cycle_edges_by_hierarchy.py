...
from file_io import write_dict_to_file, write_pairs_to_file
from helper_funs import dir_tail_name
from measures import report_performance
from remove_cycle_edges_by_hierarchy_BF import remove_cycle_edges_BF_iteratively
from remove_cycle_edges_by_hierarchy_greedy import scc_based_to_remove_cycle_edges_iteratively
...
def remove_cycle_edges_strategies(graph_file, nodes_score_dict, score_name="socialagony", nodetype=int):
    ...
    # greedy
    e1 = scc_based_to_remove_cycle_edges_iteratively(g, nodes_score_dict)
    g = nx.read_edgelist(graph_file, create_using=nx.DiGraph(), nodetype=nodetype)
    # forward
    e2 = remove_cycle_edges_BF_iteratively(g, nodes_score_dict, is_Forward=True, score_name=score_name)
    # backward
    g = nx.read_edgelist(graph_file, create_using=nx.DiGraph(), nodetype=nodetype)
    e3 = remove_cycle_edges_BF_iteratively(g, nodes_score_dict, is_Forward=False, score_name=score_name)
    return e1, e2, e3
...
def computing_hierarchy(graph_file, players_score_func_name, nodetype=int):
    if players_score_func_name == "socialagony":
        ...
        if False:
            ...
            print(f"load pre-computed socialagony from: {agony_file}")
    ...
    if players_score_func_name == "pagerank":
        ...
        if True:
            ...
            players = graphbased_trueskill(g)
            print(f"write trueskill to file: {output_file}")
            write_dict_to_file(players, output_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--score_name", default="pagerank",
                        help="nodes score function: trueskill, socialagony, ensembling, pagerank...")
