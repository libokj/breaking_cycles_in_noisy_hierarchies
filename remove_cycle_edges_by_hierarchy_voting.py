...
def remove_cycle_edges_by_agony_iteratively(sccs, edges_score, edges_to_be_removed):
    while True:
        graph = sccs.pop()
        pair_max_agony = None
        max_agony = -1
        ...
        # print("graph: (%d, %d), edge to be removed: %s, agony: %0.4f" % (graph.number_of_nodes(), 
        # graph.number_of_edges(), pair_max_agony, max_agony)) 
        graph.remove_edges_from([pair_max_agony])
        # print("graph: (%d, %d), edge to be removed: %s" % (graph.number_of_nodes(), graph.number_of_edges(), 
        # pair_max_agony)) 
...
def scc_based_to_remove_cycle_edges_iteratively(g, edges_score):
    big_sccs = get_big_sccs(g)
    if len(big_sccs) == 0:
        print(f"After removal of self loop edgs: {nx.is_directed_acyclic_graph(g)}")
        return []
    edges_to_be_removed = []
    remove_cycle_edges_by_agony_iteratively(big_sccs, edges_score, edges_to_be_removed)
...
def remove_cycle_edges_heuristic(graph_file, edges_score, nodetype=int):
    ...
    edges_to_be_removed = scc_based_to_remove_cycle_edges_iteratively(g, edges_score)
