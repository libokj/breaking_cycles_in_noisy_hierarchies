...
def dir_tail_name(file_name):
    ...
    print(f"dir name: {dir_name}, file_name: {tail}")
...
def normalize_dict(d, method="min_max"):
    ...
    if method == "min_max":
        min_v = min(values)
        max_v = max(values)
        return {key: (value - min_v) * 1.0 / (max_v - min_v) for key, value in d.items()}
    elif method == "z_score":
        mean = np.mean(values)
        std = np.std(values)
        return {key: (value - mean) * 1.0 / std for key, value in d.items()}
