...
def remove_cycle_edges_by_ranking_score_iteratively(sccs, players, edges_to_be_removed, is_Forward):
    while True:
        graph = sccs.pop()
        node_scores_dict = {}
        ...
        # node_scores = [(node, players[node]) for node in graph.nodes()]
...
def scc_based_to_remove_cycle_edges_iteratively(g, nodes_score, is_Forward):
    big_sccs = get_big_sccs(g)
    if len(big_sccs) == 0:
        print(f"After removal of self loop edgs: {nx.is_directed_acyclic_graph(g)}")
        return []
    scc_nodes_score_dict = scores_of_nodes_in_scc(big_sccs, nodes_score)
    edges_to_be_removed = []
    remove_cycle_edges_by_ranking_score_iteratively(big_sccs, scc_nodes_score_dict, edges_to_be_removed, is_Forward)
    # print(" # edges to be removed: %d" % len(edges_to_be_removed))
    return edges_to_be_removed


def remove_cycle_edges_BF_iteratively(g, players, is_Forward=True, score_name="socialagony"):
    from remove_self_loops import remove_self_loops_from_graph
    self_loops = remove_self_loops_from_graph(g)
    edges_to_be_removed = scc_based_to_remove_cycle_edges_iteratively(g, players, is_Forward)
