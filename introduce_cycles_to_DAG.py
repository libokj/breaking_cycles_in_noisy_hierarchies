	...
	while number < number_of_edges:
		...
		if nx.has_path(g, v,u):
			...
			if length <= path_length:
				...
				number += 1
	print(f"# extra edges added with path length <= {int(path_length)}: {len(extra_edges)}")
	return extra_edges
...
def add_extra_edges(g, number_of_edges):
	...
	print(f"# extra edges added (path lenght unconstrainted): {len(extra_edges)}")
	return extra_edges	
...
def add_cycle_edges(g, num_extra_edges, path_length = 1):
	if path_length == 1:
		...
		print(f"# extra edges added by length = {int(path_length)}: {len(extra_edges)}")
		return extra_edges
	else:
...
def introduce_cycles_2_DAG(graph_file, num_extra_edges, path_length):
	...
	if path_length <= 0:
		...
		print(f"path length: {int(path_length)}")
	...
	print(f"extra edges saved in: {extra_edges_file}")
	print(f"graph with extra edges saved in: {graph_with_extra_edges_file}")
